...
from schemas import LLMLogRecord, ToolCall
...
def _extract_answer(doc: Dict[str, Any]) -> Optional[str]:
    ...
    if isinstance(out, dict):
        ...
        # Optionally include URLs and titles
        chunks = []
        urls = out.get("urls")
...
def _get_tool_calls(doc: Dict[str, Any]) -> Optional[str]:
...
def parse_log_file(path: str | Path) -> LLMLogRecord:
    ...
    tool_calls = _get_tool_calls(doc)
    ...
    return LLMLogRecord(
        ...
        tool_calls=str(tool_calls) if tool_calls is not None else None,
...
if __name__ == '__main__':
    ...
    rec = parse_log_file(path='logs/wikipeida_20251115_055007_4e5a23.json')

    print(rec.filepath)
    print(rec.user_prompt)
    print(rec.total_input_tokens)